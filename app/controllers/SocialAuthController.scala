      ...
      case Some(p: SocialProvider with CommonSocialProfileBuilder) =>
        p.authenticate().flatMap {
          ...
          case Right(authInfo) => for {
            ...
            userRepository.isUserExist(user.loginInfo.providerKey).map(isExist =>
              if (!isExist) {
                userRepository.create(user.firstName.getOrElse("no name"), user.lastName.getOrElse("mo last name"), user.email.getOrElse("mo email"), user.loginInfo.providerKey)
              }
            )
            result
          }
